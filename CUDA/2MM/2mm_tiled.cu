...
__global__ void mm2_kernel1_tiled(int ni,int nj,int nk, DATA_TYPE alpha, const DATA_TYPE* __restrict__ A, const DATA_TYPE* __restrict__ B, DATA_TYPE* __restrict__ tmp)
{
    ...
    shA[threadIdx.y][threadIdx.x] = (row < ni && aCol < nk) ? A[row * NK + aCol] : 0;

    // load B‑tile (note: B is NK×NJ)
    int bRow = t + threadIdx.y;
    shB[threadIdx.y][threadIdx.x] = (bRow < nk && col < nj) ? B[bRow * NJ + col] : 0;
...
{
    __shared__ DATA_TYPE shT[TILE][TILE];
    ...
    shT[threadIdx.y][threadIdx.x] = (row < ni && tCol < nj) ? tmp[row * NJ + tCol] : 0;

    // tile from C (NJ×NL)
    int cRow = t + threadIdx.y;
    shC[threadIdx.y][threadIdx.x] = (cRow < nj && col < nl) ? C[cRow * NL + col] : 0;
    ...
    DATA_TYPE POLYBENCH_2D(D,NI,NL,ni,nl), DATA_TYPE POLYBENCH_2D(D_outputFromGpu,NI,NL,ni,nl))
{
    ...
    polybench_start_instruments;
    mm2_kernel1_tiled<<<grid1, block>>>(ni,nj,nk,alpha,A_gpu,B_gpu,tmp_gpu);
