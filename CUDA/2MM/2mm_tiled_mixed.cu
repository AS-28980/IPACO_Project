...
__global__ void mm2_kernel1(int ni, int nj, int nk, int nl, DATA_TYPE alpha, DATA_TYPE beta, DATA_TYPE *tmp, const DATA_TYPE *A, const DATA_TYPE *B){
    ...
    {
        int tiled_k = t * TILE_DIM + threadIdx.x;
        int tiled_kT = t * TILE_DIM + threadIdx.y;

        As[threadIdx.y][threadIdx.x] =
            (row < ni && tiled_k < nk) ? A[row * nk + tiled_k] : 0.0;

        Bs[threadIdx.y][threadIdx.x] =
            (col < nj && tiled_kT < nk) ? B[tiled_kT * nj + col] : 0.0;
        ...
        if ((warp_id % 8) != 0)
        {
            ...
            {
                acc32 = fmaf(As[threadIdx.y][k], Bs[k][threadIdx.x], acc32);
            }
        }
        else
        {
            ...
            for (int k = 0; k < TILE_DIM; ++k)
            {
                acc64 = fma((double)As[threadIdx.y][k], (double)Bs[k][threadIdx.x], acc64);
            }
        }
...
__global__ void mm2_kernel2(int ni, int nj, int nk, int nl, DATA_TYPE alpha, DATA_TYPE beta, const DATA_TYPE *tmp, const DATA_TYPE *C, DATA_TYPE *D){
    ...
    for (int t = 0; t < (nj + TILE_DIM - 1) / TILE_DIM; ++t)
    {
        ...
        else
        {
            ...
            for (int k = 0; k < TILE_DIM; ++k)
            {
                acc64 = fma((double)Ts[threadIdx.y][k], (double)Cs[k][threadIdx.x], acc64);
            }
        }
    ...
    if (row < ni && col < nl)
    {
        ...
        {
            double res = fma((double)D[row * nl + col], (double)beta, 0.0);
            D[row * nl + col] = (float)(res + acc64);
        }
    }
}
