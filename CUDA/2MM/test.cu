...
template<typename T>
__global__ void mm2_kernel1_tiled_slice(int ni,int nj,int nk, T alpha, const T* __restrict__ A, const T* __restrict__ B, T* __restrict__ tmp, int row_offset)
...
template<typename T>
__global__ void mm2_kernel2_tiled_slice(int ni,int nj,int nk,int nl, T beta, const T* __restrict__ tmp, const T* __restrict__ C, T* __restrict__ D, int row_offset)
...
__global__ void downcast_D64_to_F32(const double* __restrict__ src64,
    float*       __restrict__ dst32,
    int start_row, int rows,
    int ld,           // leading dimension (= NL)
    int cols)         // (= NL)
{
...
void mm2Cuda_twoStreams(int ni,int nj,int nk,int nl, float alpha32,float beta32, float*  A_h, float*  B_h, float*  C_h, float*  D_h, float*  D_out_h)
{
    ...
    float  *A_f=dmalloc<float>(szA_f), *B_f=dmalloc<float>(szB_f),
    *C_f=dmalloc<float>(szC_f), *D_f=dmalloc<float>(szD_f),
    *T_f=dmalloc<float>(szT_f);

    double *A_d=dmalloc<double>(szA_d), *B_d=dmalloc<double>(szB_d),
    *C_d=dmalloc<double>(szC_d), *D_d=dmalloc<double>(szD_d),
    *T_d=dmalloc<double>(szT_d);
    ...
    /* ---- cheap cast float→double (unchanged) -------------------------- */
    int threads=256;
    ...
    /* ------------- split rows: first half FP32, second half FP64 ------- */
    const int mid = ni/8;            // ⌊NI / 8⌋
    ...
    mm2_kernel1_tiled_slice<float><<<g32_ab,block,0,s32>>>(
    ni,nj,nk, alpha32, A_f,B_f,T_f, /*row_offset=*/0);

    mm2_kernel2_tiled_slice<float><<<g32_dc,block,0,s32>>>(
    ni,nj,nk,nl, beta32, T_f,C_f,D_f, /*row_offset=*/0);

    /* ----------------  FP64 half –– rows mid … NI‑1  ------------------- */
    const double alpha64 = static_cast<double>(alpha32);
    const double beta64  = static_cast<double>(beta32);

    mm2_kernel1_tiled_slice<double><<<g64_ab,block,0,s64>>>(
    ni,nj,nk, alpha64, A_d,B_d,T_d, /*row_offset=*/mid);

    mm2_kernel2_tiled_slice<double><<<g64_dc,block,0,s64>>>(
    ni,nj,nk,nl, beta64, T_d,C_d,D_d, /*row_offset=*/mid);
    ...
    int threadsCast = 256;
    downcast_D64_to_F32<<<(elems64+threadsCast-1)/threadsCast, threadsCast>>>(
            D_d, D_f, mid, rows64, nl, nl);

    cudaMemcpy(D_out_h, D_f, szD_f, cudaMemcpyDeviceToHost);

    /* ----------------  clean‑up  --------------------------------------- */
    cudaFree(A_f); cudaFree(B_f); cudaFree(C_f); cudaFree(D_f); cudaFree(T_f);
    cudaFree(A_d); cudaFree(B_d); cudaFree(C_d); cudaFree(D_d); cudaFree(T_d);
...
/* ------------------------------------------------------------------------- */
/*  (Your old helpers – init_array, mm2_cpu, etc. – go here unchanged.)      */
/* ------------------------------------------------------------------------- */
