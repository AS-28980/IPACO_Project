	...
	{  
		...
		for(k=0; k < _PB_NK; k++)
		{
            ...
            for(int k=0; k < _PB_NK; k++)
                res += (double)A[i * NK + k] *(double)B[k * NJ + j];
            E[i * NJ + j] = static_cast<float>(res);
	...
	{   
        ...
        if(warp_id%2==0){
           ...
           for(k=0; k < _PB_NM; k++)
            ...
            {
                res += (double)C[i * NM + k] * (double)D[k * NL +j];
            }
            F[i * NL + j] = static_cast<float>(res);
        }
...
__global__ void mm3_kernel3(int ni, int nj, int nk, int nl, int nm, DATA_TYPE *E, DATA_TYPE *F, DATA_TYPE *G)
{
	...
	{   
        ...
        if(warp_id%2 == 0){
            ...
            for(k=0; k < _PB_NJ; k++)
            {
                res += (double)E[i * NJ + k] * (double)F[k * NL + j];
            }
            G[i * NL + j] = static_cast<float>(res);
